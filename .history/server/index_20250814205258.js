...
// user: myanimelistadm
// password: bycmlla@dm
// MySQL root password: x6FPJ3oKX2>:957
// MySQL user: myanimelistuser
// MySQL user password: JT1S3=:8xaD5uKR

const con = mysql.createConnection({
  host: "177.66.203.139",
  port: 3306,
  user: "myanimelistuser",
  password: "JT1S3=:8xaD5uKR", // a senha dele
...
app.post(
  ...
  async (req, res) => {
    try {
      const { title, category, year, description, rating, genre, authors } =
        req.body;
      ...
      con.query(
        ...
        async (err, result) => {
          ...
          if (additionalImages.length > 0) {
            ...
            const values = additionalImages.map((file) => [
              obraId,
              file.filename,
            ]);
              ...
              try {
                ...
                con.query(updateSql, [dominantColor, obraId], (err3) => {
                  if (err3) {
                    ...
                    return res
                      .status(500)
                      .send("Erro ao atualizar cor dominante");
                  }

                  return res
                    .status(201)
                    .send("Obra, imagens e cor inseridas com sucesso");
                });
              } catch (colorErr) {
            ...
            return res
              .status(201)
              .send("Obra inserida com sucesso (sem imagens)");
          }
        }
      );
    } catch (err) {
