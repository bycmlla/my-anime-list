      ...
      pool.query(
        ...
        async (err, result) => {
          ...
          if (additionalImages.length > 0) {
            ...
            pool.query(sqlImages, [values], async (err2) => {
              if (err2) {
              ...
              try {
                ...
                pool.query(updateSql, [dominantColor, obraId], (err3) => {
                  if (err3) {
...
app.get("/obras", (req, res) => {
  pool.query("SELECT * FROM obras", (err, results) => {
    if (err) {
...
app.get("/obras/:id", (req, res) => {
  ...
  pool.query(sqlObra, [id], (err, obraResult) => {
    ...
    pool.query(sqlImagens, [id], (err2, imagensResult) => {
      if (err2) {
